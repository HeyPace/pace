# Phase 2AZ — Capability Roadmap & Selection

## 1. Baseline & Current State
- **Authoritative Baseline Commit**: `b921f0c` (`feat: add semantic progress indicator enumeration`)
- **Branch**: `q-secure-foundation`
- **Working Tree**: Clean
- **Current Authoritative Capability Count**: **47 capabilities**

---

## 2. Authoritative Capability Inventory (47 Registered Capabilities)

### Level 0: Read-Only Discovery & Observation (24 capabilities)
- `system.running_apps`
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **`ui.list_level_indicators`** | **Level 0** | `AXLevelIndicator`, `AXRelevanceIndicator` | `kAXValueAttribute`, `kAXMinValueAttribute`, `kAXMaxValueAttribute`, `kAXWarningValueAttribute`, `kAXCriticalValueAttribute`, `kAXTitleAttribute`, `kAXDescriptionAttribute`, `kAXIdentifierAttribute`, `kAXEnabledAttribute` | **20/20** | **15/15** | **15/15** | **15/15** | **10/10** | **10/10** | **5/5** | **5/5** | **5/5** | **100/100** |
...
**Selected Capability**: `ui.list_level_indicators`
- **Domain**: `ui`
- **Security Level**: `Level 0 (Read-Only)`
- **Why it wins**:
  1. **Strict Semantic AX Authority**: Supported directly by `AXLevelIndicator` (`kAXLevelIndicatorRole`) and `AXRelevanceIndicator` (`kAXRelevanceIndicatorRole`) with native attributes (`kAXValueAttribute`, `kAXMinValueAttribute`, `kAXMaxValueAttribute`, `kAXWarningValueAttribute`, `kAXCriticalValueAttribute`).
  ...
  3. **High Agent Usefulness**: Level indicators represent ratings (stars), battery levels, signal meters, warning/critical capacity bars, and relevance indicators across macOS applications.
  4. **Closed-Loop Verification**: Verified via `QVerificationStrategy.levelIndicatorEnumerationSucceeded(applicationName:indicatorCount:)`.
  5. **Clean AppKit Testability**: Directly testable against real AppKit `NSLevelIndicator` controls across discrete, continuous, capacity, and rating styles.

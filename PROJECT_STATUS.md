...
**Docs:** `AGENTS.md` (canonical agent instructions), `SETUP_LOCAL.md`, `docs/development/key-files.md`, `docs/development/info-plist-switches.md`, `docs/knowledge/learnings/` (learning roadmap — every novel framework/algorithm/pattern), `docs/product/brand/`.
...
- **2026-07-25:** Cancelled the unimplemented `durable-working-runtime`
  ...
  requires a fresh bounded change.
...
- **2026-07-13 (companion closeout):** Always-On Companion Mode gained a real opt-in AVFoundation/Vision camera source, user-taught object tracking, and a bundled local pre-STT Core ML wake path. `PaceWakeWordClassifier` uses exact labels `hey_pace` and `background`; missing or invalid assets fail closed before STT. Silent cards and spoken interventions are separately default-off and now flow through intervention/restraint policy; routine promotion requires three unique supporting observations. The owner explicitly directed "push through," accepting the remaining unmeasured live/hardware and manual Xcode `Cmd+R` risk. No hardware threshold or manual run is claimed as performed; the dogfood table remains a release follow-up.
- **Active plans:**
  - `docs/current/plans/autonomous-companion-consolidation.md` — canonical handoff for the remaining product-level gaps: persistent goal/activity understanding, shared proactive prioritization, outcome-driven learning, background-work UX, surface consolidation, and live proof through five acceptance stories.
  - `docs/current/plans/pace-tuned-model-v1.md` — collection now default-ON; LoRA pending accrued turn volume (filter by `plannerProvenance` before training a shippable model).
- **Test suite:** the last historical green count was ~1435 test cases via `scripts/test-pace.sh`; the current 2026-07-17 Xcode 27 beta baseline is compile-blocked as recorded under **Blocked** below. CI runs the full suite on every push/PR via `.github/workflows/ci.yml` — pinned `macos-26` runner (the `macos-latest` pool serves macOS 15 images that cannot run macOS-26-target tests), serial execution (parallel workers crash on the CI image), `-retry-tests-on-failure -test-iterations 3` for the timing-sensitive tail, a zero-tests-executed guard, and 3 hardware-bound tests gated behind `TEST_RUNNER_PACE_CI` (they still run locally). Coverage is opt-in via workflow_dispatch. Local isolated-DerivedData builds require the Metal Toolchain (`xcodebuild -downloadComponent MetalToolchain`) because `mlx-swift` compiles Metal shaders.
...
### Always-On Companion Mode (opt-in, default OFF)

- Typed provenance-bearing world observations, bounded local persistence, exact last-seen/change/presence queries, correction, decay, clearing, and conservative three-support routine promotion.
...
- `scripts/train-pace-tuned-model.sh` + eval gate docs in `docs/current/plans/pace-tuned-model-v1.md`.

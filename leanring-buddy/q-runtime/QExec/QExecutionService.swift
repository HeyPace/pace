...
public final class QExecutionService: QExecutionProvider, @unchecked Sendable {
    ...
    public func executeAction(
        ...
        case "ui.read_element_required_state":
            result = await executeReadElementRequiredState(request: request)
        ...
        case "ui.list_table_columns":
            result = await executeListTableColumns(request: request)
        case "ui.list_table_row_headers":
            result = await executeListTableRowHeaders(request: request)
    ...
    /// Phase 2BL: semantic element attribute name enumeration (Level 0, read-only, purely
    /// observational). Resolves a semantically-identified element and reads its supported
    /// Accessibility ATTRIBUTE names via `AXUIElementCopyAttributeNames` — never any attribute's
    /// actual VALUE. Discovered attribute names are DATA, not authorization: discovering that a
    /// name like `"AXValue"` exists never itself grants any capability to read that attribute's
    /// value; a subsequent value read must independently go through an existing, approved
    /// semantic read capability (e.g. `ui.read_element_value`) and that capability's own full
    /// role/privacy/security policy, completely unaffected by this capability ever having been
    /// called. Every `QAXInteractionError` failure mode is caught here and converted into a
    /// deterministic, non-throwing `QActionResult`; this method never fabricates success.
    private func executeListElementAttributes(request: QActionRequest) async -> QActionResult {
        guard let applicationName = request.parameters["applicationName"], !applicationName.isEmpty else {
            return QActionResult(
                actionId: request.actionId,
                success: false,
        ...
        do {
            ...
            var outputData: [String: String] = [
                ...
                "matchIdentifier": identifier ?? "",
                "matchTitle": title ?? "",
                "attributeCount": "\(attributes.attributeNames.count)"
            ]
            for (index, attributeName) in attributes.attributeNames.enumerated() {
                outputData["attribute\(index)"] = attributeName
            }
            return QActionResult(
                actionId: request.actionId,
                success: true,
                summary: "Observed \(attributes.attributeNames.count) supported attribute name(s) for \(role) element in \(applicationName): \(attributes.attributeNames.joined(separator: ", ")). This is observation only — no attribute value was read, and none of these names constitute authorization to read one later.",
                outputData: outputData
            )
        } catch let axError as QAXInteractionError {
            return QActionResult(
    ...
    /// Phase 2BQ: semantic element required-state read (Level 0, read-only, purely observational).
    /// Resolves a semantically-identified element and reads its `AXRequired` attribute — never
    /// pressing, focusing, or mutating anything. Genuine absence is never an error, but a genuine
    /// read failure or a malformed (non-Boolean) value fails the whole read closed (see
    /// `QBridgeAccessibility.readElementRequiredState`'s own documentation for the full
    /// rationale). Every `QAXInteractionError` failure mode is caught here and converted into a
    /// deterministic, non-throwing `QActionResult`; this method never fabricates success and never
    /// downgrades an absent value to `false`.
    private func executeReadElementRequiredState(request: QActionRequest) async -> QActionResult {
        guard let applicationName = request.parameters["applicationName"], !applicationName.isEmpty else {
            return QActionResult(
                actionId: request.actionId,
                success: false,
        ...
        do {
            ...
            let outputData: [String: String] = [
                ...
                "matchIdentifier": identifier ?? "",
                "matchTitle": title ?? "",
                "hasRequiredState": metadata.isRequired != nil ? "true" : "false",
                "isRequired": metadata.isRequired.map { $0 ? "true" : "false" } ?? ""
            ]
            let requiredDescription = metadata.isRequired.map { "\($0)" } ?? "unavailable"
            return QActionResult(
                actionId: request.actionId,
                success: true,
                summary: "Observed required-field state for \(role) element in \(applicationName): isRequired=\(requiredDescription). This is observation only — no field was modified, and this observation does not constitute authorization to modify it later.",
                outputData: outputData
            )
        } catch let axError as QAXInteractionError {
            return QActionResult(
            ...
            return QActionResult(
                actionId: request.actionId,
                success: false,
                summary: "Unexpected error while reading element required state: \(error.localizedDescription)",
                error: "AX_UNEXPECTED_ERROR"
            )
        }
    }

    /// Phase 2BR: semantic element protected-content state read (Level 0, read-only, purely
    /// observational). Resolves a semantically-identified element and reads its
    /// `AXContainsProtectedContent` attribute — never pressing, focusing, or mutating anything,
    /// and never reading any attribute other than this one. Genuine absence is never an error, but
    /// a genuine read failure or a malformed (non-Boolean) value fails the whole read closed (see
    /// `QBridgeAccessibility.readElementProtectedContentState`'s own documentation for the full
    /// rationale). This capability NEVER exposes the protected content itself — only whether it
    /// exists — and `outputData` here carries only the boolean fact and the caller's own
    /// already-supplied identity, never any content. Every `QAXInteractionError` failure mode is
    /// caught here and converted into a deterministic, non-throwing `QActionResult`; this method
    /// never fabricates success and never downgrades an absent value to `false`.
    private func executeReadElementProtectedContentState(request: QActionRequest) async -> QActionResult {
        guard let applicationName = request.parameters["applicationName"], !applicationName.isEmpty else {
            return QActionResult(
                actionId: request.actionId,
                success: false,
        ...
        do {
            ...
            let outputData: [String: String] = [
                ...
                "matchIdentifier": identifier ?? "",
                "matchTitle": title ?? "",
                "hasProtectedContentState": metadata.isProtectedContent != nil ? "true" : "false",
                "isProtectedContent": metadata.isProtectedContent.map { $0 ? "true" : "false" } ?? ""
            ]
            let protectedContentDescription = metadata.isProtectedContent.map { "\($0)" } ?? "unavailable"
            return QActionResult(
                actionId: request.actionId,
                success: true,
                summary: "Observed protected-content state for \(role) element in \(applicationName): isProtectedContent=\(protectedContentDescription). This is observation only — the protected content itself was never read, and this observation does not constitute authorization to read it later.",
                outputData: outputData
            )
        } catch let axError as QAXInteractionError {
            return QActionResult(
    ...
    /// Phase 2BJ: semantic element range read (Level 0, read-only, zero mutation). Resolves a
    /// semantically-identified `AXSlider`/`AXIncrementor`/`AXSplitter` element and reads its
    /// authoritative `kAXMinValueAttribute`/`kAXMaxValueAttribute`/current `kAXValueAttribute`
    /// (all required) plus optional `kAXValueIncrementAttribute`. Directly improves the practical
    /// safety of the already-shipped `ui.set_slider_value`/`ui.step_incrementor`/
    /// `ui.set_splitter_position` Level 2 mutations by letting the model learn valid bounds
    /// before proposing a value for human approval. Every `QAXInteractionError` failure mode is
    /// caught here and converted into a deterministic, non-throwing `QActionResult`; this method
    /// never fabricates success, never clamps, never repairs an invalid range.
    private func executeReadElementRange(request: QActionRequest) async -> QActionResult {
        guard let applicationName = request.parameters["applicationName"], !applicationName.isEmpty else {
            return QActionResult(
                actionId: request.actionId,
                success: false,
        ...
        do {
            ...
            if let valueIncrement = range.valueIncrement {
                outputData["valueIncrement"] = "\(valueIncrement)"
            }
            return QActionResult(
                actionId: request.actionId,
                success: true,
                summary: "Read \(role) element range in \(applicationName): min=\(range.minValue) max=\(range.maxValue) current=\(range.currentValue)",
                outputData: outputData
            )
        } catch let axError as QAXInteractionError {
            return QActionResult(
    ...
    private func executeReadElementValueDescription(request: QActionRequest) async -> QActionResult {
        guard let applicationName = request.parameters["applicationName"], !applicationName.isEmpty else {
            return QActionResult(
                actionId: request.actionId,
                success: false,
        ...
        do {
            ...
            let hasValueDescription = metadata != nil
            let valueDescription = metadata?.valueDescription ?? ""
            let outputData: [String: String] = [
                "applicationName": applicationName,
                "role": role,
                "matchIdentifier": identifier ?? "",
                ...
                "elementIdentifier": metadata?.elementIdentifier ?? "",
                "elementTitle": metadata?.elementTitle ?? "",
                "hasValueDescription": hasValueDescription ? "true" : "false",
                "valueDescription": valueDescription
            ]
            let summaryDescription = hasValueDescription ? "\"\(valueDescription)\"" : "unavailable"
            return QActionResult(
                actionId: request.actionId,
                success: true,
                summary: "Observed value description for \(role) element in \(applicationName): \(summaryDescription). This is observation only — no value was set, and this observation does not constitute authorization to set the element's value later.",
                outputData: outputData
            )
        } catch let axError as QAXInteractionError {
            return QActionResult(
    ...
    /// Phase 2CB: semantic element role description read — Level 0 (read-only, no approval, no
    /// mutation authority). Reads `kAXRoleDescriptionAttribute` — the SDK's own localized,
    /// human-readable explanation of an element's type/purpose (e.g. "push button"), distinct
    /// from both the raw `kAXRoleAttribute` internal role string and `kAXValueDescriptionAttribute`
    /// (a description of the element's current VALUE, never its type). Reuses
    ...
    /// already do. Unlike that sibling capability, this attribute has NO valid-absence or
    /// valid-empty case (documented "Required for all elements"), so every `QAXInteractionError`
    /// failure mode — disallowed/secure role, missing criteria, permission absence, application/
    /// target absence or ambiguity, a stale target, a genuine AX read failure, a malformed
    /// CFType, a genuinely empty string, or an oversized string — is caught here and converted
    /// into a deterministic, non-throwing `QActionResult`; this method never fabricates success
    /// and never derives a description from `kAXRoleAttribute`. Only the bounded role-description
    /// string and non-secret targeting metadata cross this method's boundary — never
    /// `kAXValueAttribute`, never a raw AXUIElement, never table/document/cell content.
    private func executeReadElementRoleDescription(request: QActionRequest) async -> QActionResult {
        guard let applicationName = request.parameters["applicationName"], !applicationName.isEmpty else {
            return QActionResult(
                actionId: request.actionId,
                success: false,
        ...
        do {
            ...
            let outputData: [String: String] = [
                ...
                "elementIdentifier": metadata.elementIdentifier ?? "",
                "elementTitle": metadata.elementTitle ?? "",
                "roleDescription": metadata.roleDescription
            ]
            return QActionResult(
                actionId: request.actionId,
                success: true,
                summary: "Observed role description for \(role) element in \(applicationName): \"\(metadata.roleDescription)\". This is observation only — no value was set, and this observation does not constitute authorization to mutate the element later.",
                outputData: outputData
            )
        } catch let axError as QAXInteractionError {
            return QActionResult(
    ...
    /// Phase 2CE: semantic element expanded state read — Level 0 (read-only, no approval, no
    /// mutation authority). Reads `kAXExpandedAttribute` — whether a disclosure triangle, popup
    /// button, combo box, or menu button is currently expanded/open, letting an agent check state
    /// before deciding to act rather than guessing or unconditionally toggling. Distinct from
    /// `ui.toggle_disclosure`'s own current-state check, which reads `kAXValueAttribute`
    /// (`AXDisclosureTriangle`'s own 0/1 convention). Reuses `QAXElementReadRolePolicy` and its
    /// `AXSecureTextField` exclusion exactly as `ui.read_element_value`/`ui.list_element_actions`/
    /// `ui.read_element_value_description`/`ui.read_element_role_description`/
    /// `ui.read_element_help_text`/`ui.read_element_placeholder_value` already do. Like
    /// `ui.read_element_required_state` (and unlike `ui.read_element_role_description`), this
    /// attribute has a valid-absence case: genuine absence (`hasExpandedState == false`) is its own
    /// valid, distinct outcome, never conflated with `false`. Every `QAXInteractionError` failure
    /// mode — disallowed/secure role, missing criteria, permission absence, application/target
    /// absence or ambiguity, a stale target, a genuine AX read failure, or a malformed
    /// (non-Boolean) CFType — is caught here and converted into a deterministic, non-throwing
    /// `QActionResult`; this method never fabricates success. Only the expanded-state Boolean and
    ...
    private func executeReadElementExpandedState(request: QActionRequest) async -> QActionResult {
        guard let applicationName = request.parameters["applicationName"], !applicationName.isEmpty else {
            return QActionResult(
                actionId: request.actionId,
                success: false,
        ...
        do {
            ...
            let outputData: [String: String] = [
                ...
                "matchIdentifier": identifier ?? "",
                "matchTitle": title ?? "",
                "hasExpandedState": metadata.isExpanded != nil ? "true" : "false",
                "isExpanded": metadata.isExpanded.map { $0 ? "true" : "false" } ?? ""
            ]
            let expandedDescription = metadata.isExpanded.map { "\($0)" } ?? "unavailable"
            return QActionResult(
                actionId: request.actionId,
                success: true,
                summary: "Observed expanded state for \(role) element in \(applicationName): isExpanded=\(expandedDescription). This is observation only — no element was modified, and this observation does not constitute authorization to modify it later.",
                outputData: outputData
            )
        } catch let axError as QAXInteractionError {
            return QActionResult(
    ...
    /// Phase 2BI: executes read-only table column-header discovery. Result summary carries an
    /// aggregate count only, never leaking individual column titles to durable storage (per-column
    /// titles are still returned in outputData for the model's own reasoning, mirroring
    /// ui.list_table_rows'/ui.list_browser_columns' identical convention).
    private func executeListTableColumns(request: QActionRequest) async -> QActionResult {
        guard let applicationName = request.parameters["applicationName"], !applicationName.isEmpty else {
            return QActionResult(
                actionId: request.actionId,
                success: false,
        ...
        do {
            ...
            for column in metadata.columns {
                ...
                outputData["column\(idx).title"] = column.title ?? ""
                outputData["column\(idx).identifier"] = column.identifier ?? ""
                outputData["column\(idx).role"] = column.role
                outputData["column\(idx).subrole"] = column.subrole ?? ""
            }

            return QActionResult(
                actionId: request.actionId,
                success: true,
                summary: "Enumerated \(metadata.columnCount) table column(s) for table in application '\(applicationName)'. This is a point-in-time snapshot only — ordering is not meaningful, and this result is never itself an actionable target; any subsequent action must independently resolve its own fresh target.",
                outputData: outputData
            )
        } catch let axError as QAXInteractionError {
            return QActionResult(
            ...
            return QActionResult(
                actionId: request.actionId,
                success: false,
                summary: "Unexpected error while listing table columns: \(error.localizedDescription)",
                error: "AX_UNEXPECTED_ERROR"
            )
        }
    }

    /// Phase 2BZ: executes read-only table row-header discovery. The direct structural mirror of
    /// `executeListTableColumns` (2BI). Result summary carries aggregate counts only, never
    /// leaking individual row-header titles into durable persistence — `outputData` is consumed
    /// only by the immediate caller and never reaches `QDurablePlanStepSnapshot` (which has no
    /// `outputData` field at all).
    private func executeListTableRowHeaders(request: QActionRequest) async -> QActionResult {
        guard let applicationName = request.parameters["applicationName"], !applicationName.isEmpty else {
            return QActionResult(
                actionId: request.actionId,
                success: false,
        ...
        do {
            ...
            var outputData: [String: String] = [
                "applicationName": applicationName,
                "role": role,
                "rowHeaderCount": "\(metadata.rowHeaderCount)"
            ]
            if let tableTitle = metadata.tableTitle {
                outputData["tableTitle"] = tableTitle
            }
            ...
            for rowHeader in metadata.rowHeaders {
                let idx = rowHeader.index
                outputData["rowHeader\(idx).index"] = "\(idx)"
                outputData["rowHeader\(idx).title"] = rowHeader.title ?? ""
                outputData["rowHeader\(idx).identifier"] = rowHeader.identifier ?? ""
                outputData["rowHeader\(idx).role"] = rowHeader.role
                outputData["rowHeader\(idx).subrole"] = rowHeader.subrole ?? ""
            }

            return QActionResult(
                actionId: request.actionId,
                success: true,
                summary: "Enumerated \(metadata.rowHeaderCount) table row header(s) for table in application '\(applicationName)'. This is a point-in-time snapshot only — ordering is not meaningful, and this result is never itself an actionable target; any subsequent action must independently resolve its own fresh target.",
                outputData: outputData
            )
        } catch let axError as QAXInteractionError {
            return QActionResult(
            ...
            return QActionResult(
                actionId: request.actionId,
                success: false,
                summary: "Unexpected error while listing table row headers: \(error.localizedDescription)",
                error: "AX_UNEXPECTED_ERROR"
            )
        }
    }

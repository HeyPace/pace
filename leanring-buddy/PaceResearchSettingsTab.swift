...
struct PaceResearchSettingsTab: View {
    ...
    private var researchTierIntroBanner: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text("When you say \"research X\" / \"look into Y\" / \"compare A vs B\", Que routes that one turn to the heavyweight model you pick here. Other turns continue to use whatever your main Planner tier is set to.")
                .font(.system(size: 12))
                .foregroundColor(DS.Colors.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
            Text("On the CLI tier, Claude Code / Codex use their own built-in WebSearch and WebFetch tools — no separate MCP server, no Composio key. The CLI does the research; Que speaks the answer.")
                .font(.system(size: 12))
                .foregroundColor(DS.Colors.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private var cliBridgeConfigurationSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Spawns the local `\(researchConfiguration.cliBridgeUpstream.rawValue)` CLI directly — no Node bridge needed. Free if you've already authenticated. (Que falls back to the local-ai bridge only for the legacy `gemini` upstream.)")
                .font(.system(size: 11))
                .foregroundColor(DS.Colors.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private var billingDisclosure: some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("Que shows the per-turn cost in the Privacy dashboard, and the token-budget cap above is a hard backstop. Cancel with \"stop researching\".")
                    .font(.system(size: 11))
                    .foregroundColor(DS.Colors.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
            }

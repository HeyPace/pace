...
struct QSemanticElementExpandedStateReadTests {
    ...
    @Test("25b. Capability count integrity: 78 → 79 was this phase's own registry-size delta; the registry has since grown further (Phase 2CF's ui.read_element_disclosure_level), so this checks the current total rather than a phase-specific snapshot — structural, confirmed by the registration test's own count assertion above")

...
struct QSemanticElementEditedStateReadTests {
    ...
    @Test("25b. Capability count integrity: 79 → 80 was Phase 2CF's own registry-size delta; the registry has since grown further (this phase's ui.read_element_edited_state), so this checks the current total rather than a phase-specific snapshot — structural, confirmed by the registration test's own count assertion above")

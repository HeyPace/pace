...
- **Text-to-Speech**: On-device **`AVSpeechSynthesizer`** via `LocalTTSClient` — the only `BuddyTTSClient` conformer. Auto-prefers Premium > Enhanced > Default English voices, then softens compact fallbacks with configurable rate/pitch/volume prosody. Cloud TTS has been removed.
...
| `LocalTTSClient.swift` | ~250 | On-device TTS via `AVSpeechSynthesizer`. The sole `BuddyTTSClient` conformer. Auto-prefers Premium > Enhanced > Default English voices, applies Info.plist prosody tuning for rate/pitch/volume/delays, and maintains its own `isCurrentlySpeakingOrPending` flag so the CompanionManager poll-loop sees playback as active from the moment `speak()` is invoked. Hops to MainActor inside the delegate callback. |
